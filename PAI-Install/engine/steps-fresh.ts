...
#
# Model provider authentication (Anthropic, Zen paid, OpenRouter, etc.) is
# managed by OpenCode via /connect — NOT via this file. OpenCode stores
# provider credentials in its own config, separate from this repository.
#
# This file is for service keys that PAI tools read directly (e.g. voice).
...
export async function runFreshInstall(
  ...
  // Step 3: Provider — default to Zen free (no API key needed)
  //
  // Provider configuration is intentionally skipped during install.
  // OpenCode ships pre-connected to OpenCode Zen with free models.
  // Users configure premium providers AFTER install via `/connect`
  // inside a running OpenCode session.
