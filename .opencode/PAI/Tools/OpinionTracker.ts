 ...
 *
 * USAGE:
 *   bun OpinionTracker.ts add "{PRINCIPAL.NAME} prefers concise responses" --category communication
 *   bun OpinionTracker.ts evidence "{PRINCIPAL.NAME} prefers concise responses" --supporting "Got positive reaction to brief answer"
 *   bun OpinionTracker.ts evidence "{PRINCIPAL.NAME} prefers concise responses" --counter "Long explanation was appreciated"
 *   bun OpinionTracker.ts list
 *   bun OpinionTracker.ts show "{PRINCIPAL.NAME} prefers concise responses"
 *
    ...
    case 'evidence': {
      ...
      try {
        ...
        if (result.needsNotification) {
          console.log('\n⚠️  SIGNIFICANT CHANGE - {PRINCIPAL.NAME} should be notified');
        }
      } catch (err) {
        console.error(`❌ ${err}`);
        process.exit(1);
...
Commands:
  ...
  evidence "statement" --confirmation "desc"   {PRINCIPAL.NAME} explicitly confirmed
  evidence "statement" --contradiction "desc"  {PRINCIPAL.NAME} explicitly contradicted
