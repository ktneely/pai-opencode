...
PAI-OpenCode runs on OpenCode, not Claude Code. Critical platform differences:

| Feature | Claude Code | OpenCode | Impact |
|---------|-------------|----------|--------|
| **Bash workdir** | `cd` persists across calls | Each `bash()` spawns NEW shell | **CRITICAL** |
| **Hooks** | Subprocess hooks | In-process plugins | Architecture |
| **Directory** | `.claude/` | `.opencode/` | Paths |
| **Agent Teams** | ✅ Available | ❌ Not implemented | Use Task tool |
...
- **BUILD/EXECUTE:** Use `workdir` parameter for all operations in that directory
- **VERIFY:** Confirm operations executed in correct location
